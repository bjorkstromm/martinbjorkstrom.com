...
* It's tightly coupled with the .NET project system, which I really don't care about in my use case.
...
# Going deeper
After reading through the NuGet.Client source on [Github](https://github.com/NuGet/NuGet.Client) and the NuGet API specification I have learned an simpler, more straightforward way to install NuGet packages. This approach doesn't rely on NuGet.PackageManagement, NuGet.Commands nor NuGet.ProjectModel, thus is completely decoupled from the .NET project system. I'm currently investigating this approach in [Depends](https://github.com/mholo65/depends) while working on a new feature, and will most probably also implement it in Cake. The approach we are going to look at is "only" depending in the following NuGet libraries:
* [`NuGet.Common`](https://www.nuget.org/packages/NuGet.Common/) - Includes some common types needed, such as NuGet's own logger abstraction.
...
5. Locate best matching assemblies with regards to the current target framework in the extracted package using `NuGet.Frameworks` and `NuGet.Packaging`
...
We begin by creating a `ISettings` by loading the default settings (i.e. `nuget.config`), this will use the default conventions as described [here](https://docs.microsoft.com/en-us/nuget/consume-packages/configuring-nuget-behavior#config-file-locations-and-uses) for locating NuGet configuration. We will need this to get the available NuGet sources to use for retrieving package dependency information. Next we will create a `SourceRepositoryProvider` and pass the `ISettings` and the default NuGet V3 `INuGetResourceProvider`'s. We can think of this as a plugin system for NuGet repositories, the documentation states the following (we will look more into what they do soon).:
...
Next, we'll just create the default null logger, but for any real usage, I strongly suggest implementing your own `ILogger` implementation. Then, iterate through all repositories provided by the `SourceRepositoryProvider`. For each `SourceRepository` we will resolve a `DependencyInfoResource`. This is where the NuGet Resource Providers comes to play. The default V3 provider registered previously will resolve this for us and make sure other resource's needed also are created. This specific resource, as the name indicates, is used for retrieving dependency information for packages. Lastly we just call the `ResolvePackage` method on the `DependencyInfoResource` to get the dependency information. We pass in a `NuGetFramework` which represent our target framework. This will automatically filter out any dependencies that are not applicable for our target framework and only focus on the best matching dependencies.
```csharp
...
```
Then what? Well, in order to get dependency information for all packages in the dependency graph, we need to recursively call `ResolvePackage` for every dependency. One such implementation could look something like the code below. We use a `HashSet<SourcePackageDependencyInfo>` for storing all packages. Note that we the `PackageIdentityComparer` as comparer. This will only compare the `PackageIdentity` (id + version), and not compare any dependencies. We will assume that if the identity match, the dependencies *should*  also match.
```csharp
var settings = Settings.LoadDefaultSettings(root: null);
var sourceRepositoryProvider = new SourceRepositoryProvider(settings, Repository.Provider.GetCoreV3());
...
As you can see in the output from the last code snippet in the previous chapter, we are depending on two different versions of `Newtonsoft.Json`. `Cake.NuGet` depends on 11.0.2 or greater while `NuGet.Packaging` depends on 9.0.1 or greater. Other packages might have even more scenarios like this. At first it would be tempting to just "pick" the highest version, but what if `Newtonsoft.Json.9.0.1` did depend on package `FooBar.0.1.0` and `Newtonsoft.Json.11.0.2` didn't? If we'd just remove duplicate packages by keeping the highest version, we'd end up with an unnecessary package (`FooBar.0.1.0`). Or, what if `NuGet.Packaging` also depended on `FooBar.0.1.0` and we'd just remove duplicate packages by removing the lower version and it's dependencies? Then we'd end up with a missing dependency. This is where `NuGet.Resolver` comes in handy. By passing the list of available package through the `NuGetResolver` we can easily filter out any duplicate dependencies. 
```csharp
var packageId = "cake.nuget";
var settings = Settings.LoadDefaultSettings(root: null);
var sourceRepositoryProvider = new SourceRepositoryProvider(settings, Repository.Provider.GetCoreV3());
...
To download the NuGet package from the repository, we'll just resolve a `DownloadResource` from the `SourceRepository` associated with the package and call `GetDownloadResourceResultAsync`. This will return a `DownloadResourceResult` which contains everything needed for obtaining information about the NuGet package and also extracting the package contents to disk.
```csharp
...
foreach (var packageToInstall in packagesToInstall)
{
...
To read information from a NuGet package, we'll use a `PackageReaderBase`. We use this to find e.g. lib items and framework items. In order to find the best matching items based on out target framework, we'll use a `FrameworkReducer` to reduce the list of available target framework in the package down to the nearest match.
```csharp
